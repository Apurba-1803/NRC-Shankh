...
import { Eye, Search, Filter as FilterIcon, Briefcase, Package, X, ChevronRight } from 'lucide-react';
...
const JobPlansTable: React.FC<JobPlansTableProps> = ({
  ...
  const [jobDetailsWithPO, setJobDetailsWithPO] = useState<JobDetailsWithPO | null>(null);
      ...
      {isModalOpen && selectedJobPlan && (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
          <div className="bg-white rounded-lg shadow-lg w-full max-w-6xl p-6 relative overflow-y-auto max-h-[90vh]">
            ...
            {/* Header */}
            <h2 className="text-xl font-semibold mb-4">Job Card Steps - {selectedJobPlan.nrcJobNo}</h2>
            ...
            <div className="grid grid-cols-2 gap-4 mb-6">
              {[
                ...
                >
                  ...
                  {item.clickable ? (
                    <button
                      onClick={() => handleJobCardClick(selectedJobPlan.nrcJobNo)}
          ...
          <div className="bg-white rounded-lg shadow-lg w-full max-w-7xl p-6 relative overflow-y-auto max-h-[90vh]">
            ...
            <div className="flex items-center space-x-3 mb-6">
              ...
              {jobDetailsWithPO?.jobDetails && (
                <span className="text-lg text-gray-600">- {jobDetailsWithPO.jobDetails.nrcJobNo}</span>
              )}
            </div>
              ...
              <div className="border-b border-gray-200 mb-6">
                <nav className="-mb-px flex space-x-8">
                  ...
                  >
                    <div className="flex items-center space-x-2">
                      <Package size={16} />
                      <span>Purchase Orders ({jobDetailsWithPO?.purchaseOrderDetails?.length || 0})</span>
                    </div>
                  </button>
                </nav>
              </div>
            ...
            {/* Job Details Tab */}
            {activeJobTab === 'job' && jobDetailsWithPO?.jobDetails && !loadingJobDetails && (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  ...
                  <div className="bg-blue-50 p-4 rounded-lg">
                    ...
                    <div className="space-y-2 text-sm">
                      <div><span className="font-medium">Style:</span> {jobDetailsWithPO.jobDetails.styleItemSKU}</div>
                      <div><span className="font-medium">Customer:</span> {jobDetailsWithPO.jobDetails.customerName}</div>
                      <div><span className="font-medium">Status:</span> {jobDetailsWithPO.jobDetails.status}</div>
                      <div><span className="font-medium">Demand:</span> {jobDetailsWithPO.jobDetails.jobDemand}</div>
                    </div>
                  </div>

                  {/* Pricing Information */}
                  <div className="bg-green-50 p-4 rounded-lg">
                    <h3 className="text-lg font-semibold text-green-800 mb-3">Pricing</h3>
                    <div className="space-y-2 text-sm">
                      <div><span className="font-medium">Latest Rate:</span> ₹{jobDetailsWithPO.jobDetails.latestRate}</div>
                      <div><span className="font-medium">Previous Rate:</span> ₹{jobDetailsWithPO.jobDetails.preRate}</div>
                      <div><span className="font-medium">Die Punch Code:</span> {jobDetailsWithPO.jobDetails.diePunchCode}</div>
                    </div>
                  </div>

                  {/* Dimensions */}
                  <div className="bg-purple-50 p-4 rounded-lg">
                    <h3 className="text-lg font-semibold text-purple-800 mb-3">Dimensions</h3>
                    <div className="space-y-2 text-sm">
                      <div><span className="font-medium">Box Dimensions:</span> {jobDetailsWithPO.jobDetails.boxDimensions}</div>
                      <div><span className="font-medium">Board Size:</span> {jobDetailsWithPO.jobDetails.boardSize}</div>
                      <div><span className="font-medium">No of Ups:</span> {jobDetailsWithPO.jobDetails.noUps}</div>
                    </div>
                  </div>

                  {/* Material Specifications */}
                  <div className="bg-orange-50 p-4 rounded-lg">
                    <h3 className="text-lg font-semibold text-orange-800 mb-3">Material Specs</h3>
                    <div className="space-y-2 text-sm">
                      <div><span className="font-medium">Flute Type:</span> {jobDetailsWithPO.jobDetails.fluteType}</div>
                      <div><span className="font-medium">Board Category:</span> {jobDetailsWithPO.jobDetails.boardCategory}</div>
                      <div><span className="font-medium">Top Face GSM:</span> {jobDetailsWithPO.jobDetails.topFaceGSM}</div>
                      <div><span className="font-medium">Fluting GSM:</span> {jobDetailsWithPO.jobDetails.flutingGSM}</div>
                      <div><span className="font-medium">Bottom Liner GSM:</span> {jobDetailsWithPO.jobDetails.bottomLinerGSM}</div>
                    </div>
                  </div>

                  {/* Colors */}
                  <div className="bg-red-50 p-4 rounded-lg">
                    <h3 className="text-lg font-semibold text-red-800 mb-3">Color Information</h3>
                    <div className="space-y-2 text-sm">
                      <div><span className="font-medium">No of Colors:</span> {jobDetailsWithPO.jobDetails.noOfColor}</div>
                    </div>
                  </div>

                  {/* Dates */}
                  <div className="bg-indigo-50 p-4 rounded-lg">
                    <h3 className="text-lg font-semibold text-indigo-800 mb-3">Important Dates</h3>
                    <div className="space-y-2 text-sm">
                      <div><span className="font-medium">Artwork Received:</span> {new Date(jobDetailsWithPO.jobDetails.artworkReceivedDate).toLocaleDateString()}</div>
                      <div><span className="font-medium">Artwork Approved:</span> {new Date(jobDetailsWithPO.jobDetails.artworkApprovedDate).toLocaleDateString()}</div>
                      <div><span className="font-medium">Shade Card Approval:</span> {new Date(jobDetailsWithPO.jobDetails.shadeCardApprovalDate).toLocaleDateString()}</div>
                    </div>
                  </div>
                </div>
              </div>
                ...
                {jobDetailsWithPO.purchaseOrderDetails.length === 0 ? (
                  ...
                  <div className="grid grid-cols-1 gap-6">
                    {jobDetailsWithPO.purchaseOrderDetails.map((po) => (
                      <div key={po.id} className="bg-gray-50 border border-gray-200 rounded-lg p-4">
                        <div className="flex justify-between items-start mb-4">
                          <h3 className="text-lg font-semibold text-gray-800">PO #{po.poNumber}</h3>
                          <span className={`px-2 py-1 text-xs font-medium rounded-full ${
                            po.status === 'created' ? 'bg-blue-100 text-blue-800' : 'bg-gray-100 text-gray-800'
                          }`}>
                            {po.status}
                          </span>
                        </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                          <div>
                            ...
                            <div className="space-y-1 text-sm">
                              <div><span className="font-medium">Customer:</span> {po.customer}</div>
                              <div><span className="font-medium">Plant:</span> {po.plant}</div>
                              <div><span className="font-medium">Unit:</span> {po.unit}</div>
                            </div>
                          </div>
                          
                          <div>
                            <h4 className="font-medium text-gray-700 mb-2">Quantities</h4>
                            <div className="space-y-1 text-sm">
                              <div><span className="font-medium">Total PO Qty:</span> {po.totalPOQuantity?.toLocaleString()}</div>
                              <div><span className="font-medium">Dispatch Qty:</span> {po.dispatchQuantity?.toLocaleString()}</div>
                              <div><span className="font-medium">Pending Qty:</span> {po.pendingQuantity?.toLocaleString()}</div>
                            </div>
                          </div>
                          ...
                          <div>
                            ...
                            <div className="space-y-1 text-sm">
                              <div><span className="font-medium">PO Date:</span> {new Date(po.poDate).toLocaleDateString()}</div>
                              <div><span className="font-medium">Delivery Date:</span> {new Date(po.deliveryDate).toLocaleDateString()}</div>
                            </div>
                          </div>
                        </div>
                      </div>
