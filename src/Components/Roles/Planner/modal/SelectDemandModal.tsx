...
const demandOptions: {
...
}[] = [
  {
    value: "high",
    label: "Urgent",
    description: "Priority scheduling, flexible machine assignment",
    uiLabel: "Urgent",
  },
  {
    value: "medium",
    label: "Regular",
    description:
      "Standard production timeline, machines required for selected steps",
    uiLabel: "Regular",
  },
];

const SelectDemandModal: React.FC<SelectDemandModalProps> = ({
  currentDemand,
  onSelect,
  onClose,
}) => {
  const [selectedOption, setSelectedOption] =
    useState<Job["jobDemand"]>(currentDemand);
      ...
      <div className="relative w-full max-w-sm mx-auto bg-white rounded-2xl shadow-2xl p-0 flex flex-col items-center">
        ...
        <div className="w-full px-8 pt-10 pb-8 flex flex-col items-center overflow-y-auto max-h-[85vh]">
          ...
          <p className="text-gray-500 text-center mb-6">
            What is the demand level of this job?
          </p>
                ...
                <div className="ml-3 flex-1">
                  ...
                  {option.value === "high" && (
                    <div className="mt-2 p-2 bg-red-100 rounded text-xs text-red-700">
                      <strong>Note:</strong> Machine assignment is flexible -
                      not all machines required
                    </div>
                  )}
                  {option.value === "medium" && (
                    <div className="mt-2 p-2 bg-[#00AEEF]/20 rounded text-xs text-[#00AEEF]">
                      <strong>Note:</strong> Machine assignment is mandatory for
                      all selected steps
                    </div>
                  )}
                </div>
