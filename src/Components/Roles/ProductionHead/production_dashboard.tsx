...
const [modalJobData, setModalJobData] = useState<Array<{ jobPlan: JobPlan; step: ProductionStep }>>([]);
...
const [isLoadingModalData, setIsLoadingModalData] = useState(false);
  ...
  const handleStatusCardClick = async (stepKey: string, status: string, stepName: string) => {
  try {
    ...
    if (stepKey === 'all') {
      ...
      jobData = await productionService.getJobsByStepAndStatus(stepKey, status);
      title = `${stepName} - ${status.charAt(0).toUpperCase() + status.slice(1)} Jobs`;
    }
          ...
          {stepData ? (
            <div className="space-y-2">
              <div className="grid grid-cols-2 gap-2 text-xs">
                {/* Completed - Clickable */}
                <button
                  onClick={() => handleStatusCardClick(step.key, 'completed', step.name)}
                ...
                <button
                  onClick={() => handleStatusCardClick(step.key, 'start', step.name)}
                  className="text-center p-2 bg-blue-100 rounded hover:bg-blue-200 transition-colors cursor-pointer"
                  disabled={isLoadingModalData}
                >
                  <p className="font-semibold text-blue-800">{stepData.start}</p>
                  <p className="text-blue-600">In Progress</p>
                </button>

                {/* Planned - Clickable */}
                <button
                  onClick={() => handleStatusCardClick(step.key, 'planned', step.name)}
                ...
                {/* Stopped - Clickable */}
                <button
                  onClick={() => handleStatusCardClick(step.key, 'stop', step.name)}
